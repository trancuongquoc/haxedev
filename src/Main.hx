import h2d.Tile;
import hxd.fmt.tiff.Data.TifFile;
...
class Main extends hxd.App {
	...
	override function init() {
		...
		var forestTile = hxd.Res.forest.toTile();
		drawLayerCDB(forestTile, 0, TILE_SIZE);
	}
	function drawLayerCDB(tiles:h2d.Tile, layer:Int, size:Int) {
		...
		// 	var dataOfLayer = layer.data.data.decode();
			// trace(dataOfLayer);
			// switch (layer.name) {
			// 	case "ground":
			// }
	...
	static function main() {
		...
		trace(dat.Data.collide.get(Full).icon);
	}
}